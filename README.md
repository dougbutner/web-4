...
### Minimal Example
	...
	function getTu() { // Returns the current Time Unit
	  ...
	  let time_unit = Math.floor((Math.floor(nowts/1000) - 1561139700) / 300);  // Divide by the length of a Time Unit in seconds
	  return time_unit;
	}//END getTu()
  
### Advanced Time Token Implementations
...
In this contract, the tokens used to do the voting are Time Tokens for the dapp [cXc.world](https://cxc.world). The contract listens for transfers of this time token to the contract, and assigns an equal amount of weight to each vote in `logup()`. To view the full contract an actions, review an implementation Douglas is developing for [cXc], particularly [ups.hpp](https://github.com/dougbutner/beta-pseudo/blob/main/real-contracts/ups.hpp), [ups.cpp](https://github.com/dougbutner/beta-pseudo/blob/main/real-contracts/ups.cpp) and [helpups.cpp](https://github.com/dougbutner/beta-pseudo/blob/main/real-contracts/helpups.cpp) To understand how Web4 concepts are used in a real-world environment, view the [explainer document](https://github.com/currentxchange/purple-explainer).
